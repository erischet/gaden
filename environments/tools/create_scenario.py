...
Chains the three scenario-authoring tools in this folder, invoking each of
them through `uv run` (every tool script here declares its own dependencies
inline via PEP 723 - see environments/tools/README.md - so `uv run` is the one
uniform way to invoke any of them, whether or not they need extra packages):
    ...
    2. generate_walls_and_obstacles.py  - derive walls/obstacle STLs from the
       copied "_inner.stl" (needs trimesh; uv resolves/caches it automatically).
...
def main():
    ...
    create_cmd = ["uv", "run", TOOLS_DIR / "create_scenario_from_raw_data.py", args.input_dir, args.name]
    if args.force:
    ...
    walls_cmd = ["uv", "run", TOOLS_DIR / "generate_walls_and_obstacles.py", inner_stl, "--thickness", str(args.thickness)]
    run_step("2/3 generate walls and obstacles", walls_cmd)

    update_cmd = ["uv", "run", TOOLS_DIR / "update_scenario_models.py", scenario_dir]
    if args.config:
